...
export async function handleAccountCredited(payload: Record<string, unknown>): Promise<boolean> {
  const data = payload as unknown as AccountCreditedPayload;
  ...
  const msg = `Dear ${displayName}, your account has been credited for your order #${order.orderNumber}, amount: KSh ${data.amount}. New outstanding balance is: KSh ${data.balance}. We hope you enjoy our services.`;
  ...
  try {
    ...
    broadcastLive(customer.id, notification.id, "WALLET_CREDIT", "Account credited", msg);
  } catch {
