...
interface AccountRefundPayload {
  customerId: string;
  orderId: string;
  orderNumber: number;
  recordId: string;
...
export async function handleAccountRefund(payload: Record<string, unknown>): Promise<boolean> {
  const data = payload as unknown as AccountRefundPayload;
  ...
  const msg = `Dear ${displayName}, a refund has been issued for your order #${order.orderNumber}, amount: KSh ${data.amount}. New outstanding balance is: KSh ${data.balance}. We're sorry for the inconvenience and hope to serve you better next time.`;
  ...
  try {
    ...
    broadcastLive(customer.id, notification.id, "WALLET_REFUND", "Refund issued", msg);
  } catch {
