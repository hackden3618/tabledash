                            ...
                            {hotels.map((hotel) => (
                                ...
                                >
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <h3 style={{ fontSize: "1.05rem", fontWeight: 600, color: "#1F2937", display: "flex", alignItems: "center", gap: "8px" }}>
                                            {hotel.name}
                                            {!hotel.isOpen && (
                                                <span style={{ fontSize: "0.65rem", fontWeight: 700, color: "#DC2626", background: "#FEE2E2", padding: "2px 8px", borderRadius: "999px", textTransform: "uppercase", letterSpacing: "0.03em" }}>
                                                    Closed
                                                </span>
                                            )}
                                        </h3>
            ...
            <div style={{ padding: "20px" }}>
                ...
                {closingCountdown !== null && (
                    <div style={{
                        ...
                        padding: "16px 18px", marginBottom: "20px", animation: "pulse 1s ease-in-out infinite",
                        textAlign: "center",
                    }}>
                        ...
                        <div style={{ fontSize: "0.85rem", color: "#991B1B" }}>
                            {selectedHotel?.name} is closing. No new orders will be accepted after the timer ends.
                        </div>
                    </div>
                )}
                                            ...
                                            <div style={{ flexShrink: 0 }}>
                                                {qty === 0 ? (
                                                    <button onClick={() => handleAddToCart(item)}
                                                        ...
                                                        style={{
                                                            border: "1px solid #1E4D36", background: isEffectivelyClosed ? "#F3F4F6" : "#EBF4F0",
                                                            color: isEffectivelyClosed ? "#9CA3AF" : "#1E4D36",
                                                            padding: "8px 14px", borderRadius: "8px", fontWeight: 700,
                                                            cursor: isEffectivelyClosed ? "not-allowed" : "pointer", opacity: isEffectivelyClosed ? 0.5 : 1,
                                                        }}>
                                                        {isEffectivelyClosed ? "Closed" : "+ Add"}
                                                    </button>
                                                ) : (
                                                    <div style={{ display: "flex", alignItems: "center", gap: "8px", background: "#EBF4F0", padding: "4px 8px", borderRadius: "8px", border: "1px solid #1E4D36" }}>
                                                        ...
                                                        <button onClick={() => updateQuantity(item.id, Math.min(qty + 1, maxQty))}
                                                            disabled={qty >= maxQty || isEffectivelyClosed}
                                                            style={{ border: "none", background: "none", fontWeight: 700, fontSize: "1.1rem", color: qty >= maxQty || isEffectivelyClosed ? "#D1D5DB" : "#1E4D36", cursor: qty >= maxQty || isEffectivelyClosed ? "not-allowed" : "pointer", padding: "0 4px" }}>+</button>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
            ...
            {totalCount > 0 && (
                <div className="sticky-bottom-bar">
                    <button onClick={onNavigateToCart}
                        ...
                        style={{ opacity: isEffectivelyClosed ? 0.6 : 1 }}>
                        {isEffectivelyClosed ? "Hotel Closed" : `View Cart ( KSh ${totalAmount} )`}
                    </button>
                </div>
            )}
