...
export const OrderTrackingPage: React.FC<OrderTrackingPageProps> = ({ orderId, onBackToHome }) => {
  ...
  useEffect(() => {
    // The #review anchor (from the "rate your meal" SMS) only exists once the
    // order has loaded and the review section has actually rendered — a plain
    // browser hash-scroll on initial page load happens before that, so it's a
    // no-op without this. Runs once order data (and thus the section) is present.
    if (!order || window.location.hash !== "#review") return;
